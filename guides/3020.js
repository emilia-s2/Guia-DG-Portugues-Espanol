...
function skilld_event(skillid, handlers, event, ent, dispatch) {
	...
	if (skillid == 120) {
		...
		if (purple && !boss_thirty) {
			handlers["text"]({"type": "text","sub_type": "message","message": "ENTRAR - SAIR - ENTRAR","message_RU": "К нему > От него > К нему"});
			// бублик перед боссом
		...
		// к нему (бублик вокруг босса) -> от него (круг перед боссом) -> [волны] -> к нему (бублик перед боссом)
			...
			handlers["text"]({"type": "text","sub_type": "message","message": "ENTRAR - SAIR - SAIR","message_RU": "К нему > От него > (К нему)"});
//			handlers['text']({"type": "text","sub_type": "message","delay": 5000,"message": "ENTRAR","message_RU": "К нему"});
	...
	if (skillid == 123) {
		...
		if (green && !boss_thirty) {
			handlers["text"]({"type": "text","sub_type": "message","message": "SAIR - ENTRAR - SAIR","message_RU": "От него > К нему > От него"});
		...
		// к нему (бублик вокруг босса) -> от него (круг перед боссом) -> от него (большой круг перед боссом)
			...
			handlers["text"]({"type": "text","sub_type": "message","message": "ENTRAR - SAIR - SAIR","message_RU": "К нему > От него > От него"});			
			// большой круг перед боссом
			spawn.circle(true, 912, 0, 200, 8, 450, 5000, 3000); // 3
			dispatch.setTimeout(() => purple = false,  2000);
...
//			handlers['text']({"type": "text","sub_type": "message","delay": 5000,"message": "SAIR","message_RU": "От него"});
	...
	if (skillid == 127) {
		if (boss_thirty) {
			handlers["text"]({ "type": "text", "sub_type": "message", "message": "Pulo | Saia", "message_RU": "Прыжок | От него" });
		} else {
			handlers["text"]({ "type": "text", "sub_type": "message", "message": "Pulo | Entra", "message_RU": "Прыжок | К нему" });
...
let debuffs_targe = {
	30209101: { message: "Relampago", message_RU: "Молния (эвейд)" },
	30209102: { message: "Bruxa",     message_RU: "Ведьма (эвейд)" }
};
...
module.exports = {
	...
	// 1 BOSS
	"s-3020-1900-104-0": [{ "type": "text", "sub_type": "message", "message": "Succao (Esquiva)", "message_RU": "Высасывание (Выйти)" }, { "type": "spawn_func", "func": "circle", "args": [true, 553, 0, 0, 15, 450, 200, 6000] }],

	// 2 BOSS
	"s-3020-1200-103-0": [{ "type": "text", "sub_type": "message", "message": "Succao (Esquiva)","message_RU": "Высасывание"}],
	...
	"s-3020-2200-128-0": [{ "type": "text", "sub_type": "message", "message": "Golfe (voar)", "message_RU": "Черкаш (подлет)" }],
	"s-3020-2200-129-0": [{ "type": "text", "sub_type": "message", "message": "Martelo Esmagador", "message_RU": "Полоса в цель" },
	...
	"s-3020-2200-133-1": [{ "type": "text", "sub_type": "message", "message": "Espalhar Voar (Donuts)", "message_RU": "Бублики" },
	...
	"s-3020-2200-135-0": [{ "type": "text", "sub_type": "message", "message": "Pular Corda", "message_RU": "Волны х5" }],

	"s-3020-2200-137-0": [{ "type": "text", "sub_type": "message", "message": "Espalhar", "message_RU": "Волна от" }],
	"s-3020-2200-139-0": [{ "type": "text", "sub_type": "message", "message": "Encolher", "message_RU": "Волна к" }],
